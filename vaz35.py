# from estimated ekf of rx power 
import pandas as pd
import numpy as np
...
file_path = r"C:\Users\SHAIK VAZEER AHAMED\OneDrive\Desktop\S_N_ES_ekf.csv"
...
plt.plot(data['S_N'], 'g*-', label='1st ekf')
plt.plot(kf_estimated_values[:, 0], 'r*-', label='KF Estimated')
plt.plot(ekf_estimated_values[:, 0], 'bo-', label='EKF Estimated')

plt.xlabel('Samples')
plt.ylabel('S_N from estimated ekf')
plt.title('1st ekf, KF-Estimated, and EKF-Estimated Values')
plt.legend()
plt.show()
