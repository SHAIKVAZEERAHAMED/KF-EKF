...
file_path = r"C:\Users\SHAIK VAZEER AHAMED\OneDrive\Desktop\csv_kf_ekf\RxPower.csv"
...
initial_state = np.array([data['RxPower'].iloc[0]])
...
for _, row in data.iterrows():
    measurement = np.array([row['RxPower']])
...
plt.plot(data['RxPower']/abs(N), 'g*-', label='Original')
plt.plot(kf_estimated_values[:, 0]/abs(N), 'bo-', label='KF Estimated')
plt.plot(ekf_estimated_values[:, 0]/abs(N), 'r*-', label='EKF Estimated')
