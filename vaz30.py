...
dim_z = 2  # Angle and intensity measurements
...
for i in range(min_length):
    ...
    if intensity1[index1] > intensity2[index2]:
        ...
        z = np.array([[angle_dataset1.iloc[i]], [intensity1[index1]]])
    else:
        selected_angle = angle_dataset2.iloc[i]
        z = np.array([[angle_dataset2.iloc[i]], [intensity2[index2]]])
...
# # Select the desired index to estimate the angle
# index = 0

# intensity1 = intensity_dataset1.iloc[index].values
# intensity2 = intensity_dataset2.iloc[index].values
...
# # Select the angle based on the radar with higher intensity
# if intensity1[index1] > intensity2[index2]:
#     selected_angle = angle_dataset1.iloc[index]
#     z = np.array([[angle_dataset1.iloc[index]]])
# else:
#     selected_angle = angle_dataset2.iloc[index]
#     z = np.array([[angle_dataset2.iloc[index]]])
...
# # Plot the selected and estimated angles
# plt.plot(time_dataset1[index], selected_angle, 'bo', label='Selected Angle')
# plt.plot(time_dataset1[index], estimated_angle, 'g+', label='Estimated Angle')
# plt.xlabel('Time')
# plt.ylabel('Angle')
# plt.title('Selected and Estimated Angle')
# plt.legend()
# plt.show()


#     # # Plot the selected and estimated angles
#     # plt.plot(time_dataset1[:index+1], selected_angle[:index+1], 'bo-', label='Selected Angle')
#     # plt.plot(time_dataset1[:index+1], [estimated_angle] * (index+1), 'r*-', label='Estimated Angle')
#     plt.plot(time_dataset1[index], selected_angle[:index+1], 'bo-', label='Selected Angle')
#     plt.plot(time_dataset1[index], [estimated_angle] * (index+1), 'r*-', label='Estimated Angle')
#     plt.xlabel('Time')
#     plt.ylabel('Angle')
#     plt.title('Selected and Estimated Angle')
#     plt.legend()
