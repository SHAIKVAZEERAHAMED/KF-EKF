...
file_path = r"C:\Users\SHAIK VAZEER AHAMED\OneDrive\Desktop\M64_ES_ekf_S_N.csv"

# Load the data from CSV file
data = pd.read_csv(file_path)
data['M64_ES_ekf_S_N'] = data['M64_ES_ekf_S_N'].str.replace('[', '').str.replace(']', '').astype(float)
...
initial_state = np.array([data['M64_ES_ekf_S_N'].iloc[0]])
...
for _, row in data.iterrows():
    measurement = np.array([row['M64_ES_ekf_S_N']])
...
plt.plot(data['M64_ES_ekf_S_N'], 'g*-', label='1st EKF-estimated')
plt.plot(kf_estimated_values[:, 0], 'bo-', label='KF Estimated')
plt.plot(ekf_estimated_values[:, 0], 'r*-', label='EKF Estimated')

plt.xlabel('Sample')
plt.ylabel('S/N for M64 for estimated ekf')
plt.title('1st EKF-estimated, KF-Estimated, and EKF-Estimated')
plt.legend()
plt.show()
