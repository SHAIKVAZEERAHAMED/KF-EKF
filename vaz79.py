...
avg_x = np.mean(fused_data[:, 0] - vision_data[:, 0])
avg_y = np.mean(fused_data[:, 1] - vision_data[:, 1])

print("Average difference in X:", avg_x)
print("Average difference in Y:", avg_y)

print("EM 0")
avg_x1 = np.mean(fused_data[:, 0] - magnetic_data[:, 0])
avg_y1 = np.mean(fused_data[:, 1] - magnetic_data[:, 1])

print("Average difference in X:", avg_x1)
print("Average difference in Y:", avg_y1)
