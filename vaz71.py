...
for measurement in vision_measurements:
    ...
    # Get the Kalman gain
    kalman_gain = kalman_filter.K
    
    # Calculate the weighted average using the Kalman gain
    fused_measurement = np.dot(kalman_gain, measurement).flatten()
...
fused_measurements_df = pd.DataFrame(fused_measurements, columns=['fused_dis_m', 'fused_ang_m', 'fused_X', 'fused_Y', 'fused_Pos(Mx)', 'fused_Pos(My)', 'fused_dis_m2', 'fused_ang_m2'])
...
print(fused_measurements_df['fused_Y'])
